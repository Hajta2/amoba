...
Board::Board(){
    ...
    std::string openGame="Kattints ide, vagy nyomj ENTERT, ha játszani szeretnél";
    ...
    std::string endGame="Kattints ide, vagy nyomj ESCAPE-t, ha ki szeretnél lépni";
...
void Board::endScreen()
{
    ...
    std::string who=std::to_string(player) + ". Játékos nyert!";
    Text * whoWon;
    gout << move_to(0,0) << color(0,0,0) << box(max_X,max_Y);
    whoWon = new Text(max_X/2-gout.twidth(who)/2,max_Y/4,gout.twidth(who),20,who);
    whoWon->draw();
